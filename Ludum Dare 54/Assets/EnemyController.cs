...
{
    public Transform motherShipPos;  // Drag your mother ship here in the inspector
    ...
    public GameObject oreChunckPrefab; // Ore chunk prefab
    ...
    private void Update()
    {
        if (motherShipPos != null)
        {
            Vector2 direction = (motherShipPos.position - transform.position).normalized;
            if (rb)
            {
    ...
    private void SpawnDebris()
    {
        ...
        {
            // Add an ore chunk!
            GameObject chunk = Instantiate(oreChunckPrefab, transform.position, Quaternion.identity);
