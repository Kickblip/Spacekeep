...
public class EnemyControllerTest : MonoBehaviour
{
    ...
    List<EnemyControllerTest> enemyNeighbors = new List<EnemyControllerTest>();
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < enemyCols.Length; i++)
        {
            if (enemyCols[i] != this.GetComponent<Collider2D>())
                enemyNeighbors.Add(enemyCols[i].gameObject.GetComponent<EnemyControllerTest>());
        }
    ...
    Vector2 Avoid()
    {
        ...
        {
            foreach (EnemyControllerTest enemy in enemyNeighbors)
            {
                if (enemy != null)
                {
